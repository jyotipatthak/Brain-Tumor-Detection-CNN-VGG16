...
2. Go to **File → Upload Notebook** → select `Brain_Tumor_Simple_Upload.ipynb`
3. Set Runtime: **Runtime → Change Runtime Type → T4 GPU**
4. Download dataset from [Kaggle](https://www.kaggle.com/datasets/navoneel/brain-mri-images-for-brain-tumor-detection)
5. Run cells top to bottom — upload ZIP when prompted in **Step 1**
...
# 4. Run the notebook
jupyter notebook Brain_Tumor_Simple_Upload.ipynb
```
...
│
├── Brain_Tumor_Simple_Upload.ipynb   ← Main notebook (run this)
│
├── brain_data/                       ← Dataset (download from Kaggle)
│   ├── yes/                          ← 155 tumor MRI images
│   └── no/                           ←  98 no-tumor MRI images
│
├── split_data/                       ← Auto-created during training
│   ├── train/
│   │   ├── yes/
│   │   └── no/
│   ├── val/
...
│       └── no/
│
├── outputs/                          ← Auto-saved during training
│   ├── sample_images.png
...
│   ├── VGG16_Fine_Tune.png
│   ├── cm_Custom_CNN.png
│   ├── cm_VGG16_Transfer_Learning.png
